...
- **When Focus is shown, drop fixed `Height="140"` / Results `Height="120"` for that surface** — use stretch + `MinHeight` floor (~180–220px Operate). Focus lives **inside** the Details drawer (Dock top + gauges below), not a third Grid star row. Step list and Details share one splitter; chrome toggles reset both rows to `*` / `0` so Absolute heights from drag cannot stick.
...
- **Focus mode:** KPI strip retained; **one** `MeasurementPlotView` docks at the top of the Details drawer with honest MinHeight; gauges/detail scroll beneath. Operator resizes list vs drawer via the single splitter.
