...
- Pseudo-code:
  ...
  - **Preview/eval:** Authoring.Core **must** call `TransferFunctionTimeBase.ElapsedMs` / `Values` (1:1, null → NaN, never drop) then `TransferFunctionGrid.RequireUniform` then `TransferFunctionFilter` — same mapping as the bench step. Successful preview publishes synthetic samples with the output ChannelKey + the RequireUniform-validated ElapsedMs vector. A mixed/NaN vector **fails** `RequireUniform` (the 1:1 NaN mapping is the fail-closed input, not a successful publish). No dataset: synthesize `elapsedMs[i] = i * TsSeconds * 1000` from the editor `Ts` field (Area 7 canned samples have no clock), then the same TimeBase → RequireUniform path. `--eval-formulas` includes TF goldens. Identification goldens are dedicated fixtures **with** `elapsedMs`; do not use timestamp-only `run-v1.json`.
