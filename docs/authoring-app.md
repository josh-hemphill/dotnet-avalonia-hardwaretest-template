...
public static class TransferFunctionGrid
{
    ...
    // Fail closed: empty or length < 2 (median(diff) undefined), missing/NaN elapsed
    // (including 1:1 NaN from TimeBase), irregular dt
    // (|dt_i - medianDt| / medianDt > DefaultTsRelativeEpsilon),
...
{
    public string InputChannel { get; set; } = string.Empty; // Save/decompile: TransferFunctionAlgorithm.InputChannelKey
    ...
    // Reads sibling Sample rows in order. Missing/NaN ElapsedMs → NaN at that index
    // (same length as Value; never drop; never coerce to 0). Then RequireUniform then Filter/FiltFilt.
...
{
    // Same length as series for both methods. StoredSample.ElapsedMs is double?; null → NaN.
...
- Pseudo-code:
  - **Import:** file picker or drop `models/*.tf.json` (`TfModelJson`: schemaVersion 1, `additionalProperties: false`, input/output keys, tsSeconds, numerator, denominator, method, timeBase=`elapsedMs`, initialConditions=`zero`). Required keys: `schemaVersion`, `tsSeconds`, `numerator`, `denominator`, `method`, `timeBase`, `inputChannelKey`, `outputChannelKey`, `initialConditions`. Fail closed: omitted required key, `TsSeconds <= 0`, empty numerator or denominator, `InitialConditions != zero`, `TimeBase != elapsedMs` (`index` / `timestamp` included), unknown method (`Filter` ≠ `filter`; method is lowercase `filter` | `filtfilt`), schemaVersion > 1 (**fail Load/apply** — do not treat v2 coefficients as v1). Normalize by dividing all `b`/`a` by `a[0]`. If `a[0] == 0` (or den empty after normalize) fail `TF_DEN_LEADING_ZERO` — do not emit NaN IIR. `Load` returns `TfImport` so `OutputChannelKey` becomes `MetricDraft.ChannelKey`.
  - **Parser boundary:** Area 11 **extends** Area 3 `FormulaParser` / `FormulaLowerer` (same types). New AST node `FilterCall(b, a, channel, method)`. Area 3 still treats `filter` / `filtfilt` as unknown; Area 11 registers the call. `Lower(FilterCall)` → `TransferFunctionAlgorithm`, never Expressions. Nested `filter` / `filtfilt` (e.g. `mean(filter(...))`, `filter(...) + 1`) **fail parse or Lower** — do not fall through to Expressions. Coefficient vectors are numeric literals (`[0.5 0.5]` or `[0.5, 0.5]`). If `a[0] == 0`, Lower fails `TF_DEN_LEADING_ZERO` (same as import); otherwise normalize `b`/`a` by `a[0]` the same as import. `TsSeconds` from bound series median dt, or a required editor field if no recording. Channel argument is an identifier from `InputChannelKeys`, not a nested expression.
  - **Save / first-recording clock:** Area 11 changes **only** `AcquireVoltageStep` to always publish Sample `ElapsedMs` (`IntervalMs * index`) even when limits and series compliance are off. Do **not** add limits to get a clock (that inverts shape-only timeseries / `MISSING_LIMITS`). Existing tests that assert `Channel, Index, Value` only (including `Three_column_sample_still_publishes_without_elapsed`) must be updated in this area. That way the **first** operator `run.json` used for MATLAB identification already has a uniform grid — ElapsedMs is not gated on a TF sibling that does not exist yet. Other Basic publishers:
    - `BitSweepAcquireStep` already always publishes Sample `ElapsedMs` (`IntervalMs * index`). Do not change its column set.
    - `PublishTimedSampleStep` already has an `ElapsedMs` column but it is optional (`NaN` when unset) and has **no** `IntervalMs`. Do not invent a clock. TF input only when the engineer set `ElapsedMs`; Save fails `TF_MISSING_ELAPSED` if that sibling’s published elapsed is NaN.
    - `PublishSeriesComplianceStep` emits Scalar summaries only — not a TF input.
    Save still emits `ApplyTransferFunctionStep` after the measure leaf for `InputChannelKey` and fails `TF_MISSING_ELAPSED` if that sibling type cannot publish `ElapsedMs` (InstrumentComponents series included). Timestamp-only legacy recordings remain fail-closed. Presentation on the TF step uses `MetricDraft.ChannelKey`. `filtfilt` is a sibling analyze (whole series), never nested inside acquire. Still never `DialogStep`. `OpenTapStepKinds` recognizes `ApplyTransferFunctionStep` for decompile (function leaf, not Presentation-exempt).
  - **Decompile:** `ApplyTransferFunctionStep` → `TransferFunctionAlgorithm` (not `RawStepNode`), restoring `InputChannelKey`, `Numerator`, `Denominator`, `TsSeconds`, and `Method`.
  - **Preview/eval:** Authoring.Core **must** call `TransferFunctionTimeBase.ElapsedMs` / `Values` (1:1, null → NaN, never drop) then `TransferFunctionGrid.RequireUniform` then `TransferFunctionFilter.Filter` or `.FiltFilt` according to `Method` — same mapping and dispatch as the bench step. Successful preview publishes synthetic samples with the output ChannelKey + the RequireUniform-validated ElapsedMs vector. A mixed/NaN vector **fails** `RequireUniform` (the 1:1 NaN mapping is the fail-closed input, not a successful publish). No dataset: synthesize `elapsedMs[i] = i * TsSeconds * 1000` from the editor `Ts` field (Area 7 canned samples have no clock), then the same TimeBase → RequireUniform path. `--eval-formulas` includes TF goldens. Identification goldens are dedicated fixtures **with** `elapsedMs`; do not use timestamp-only `run-v1.json`.
  - **Bench Run:** `ApplyTransferFunctionStep` reads sibling Sample rows in published order. Missing `ElapsedMs` column or NaN cell → NaN at that index (**same length as Value**; never drop). Then the **same** `RequireUniform` then Filter/FiltFilt. Fail verdict on grid errors — no index-order fallback, no coerce-to-zero.
  - **MATLAB snippet** (docs only, not a shipped toolbox): `jsondecode` `run.json`, select samples by `metricKey` (fallback `channel`), **`error` if any selected sample lacks finite `elapsedMs` or length < 2**, `Ts = median(diff(elapsedMs))/1000`, obtain discrete `b`/`a` from `butter` (design) or `tfest(iddata(u,x,Ts),…)` (identify — needs a known stimulus `x`, not `u` alone), `c2d` **if the model is continuous**, then `jsonencode` `TfModelJson`. Golden: MATLAB `filter(b,a,u)` vector checked in beside the JSON; .NET `Filter` max abs error below a named epsilon (e.g. 1e-9 relative 1e-6).
...
- Tests: DF-II impulse response of a checked-in biquad matches golden; `FiltFilt` matches checked-in MATLAB `filtfilt` vector; missing `ElapsedMs` fails **both** preview (via `TimeBase` → `RequireUniform`) and `ApplyTransferFunctionStep` (1:1 NaN mapping, no index-order, no drop, no coerce-to-zero); mixed series (one null `ElapsedMs`) keeps length and fails `RequireUniform` on preview **and** the step; no-dataset TF preview uses `elapsedMs[i] = i * TsSeconds * 1000` then TimeBase → RequireUniform (does not stay all-NaN); empty series and single-sample series fail `RequireUniform` (length < 2); irregular dt fails (`|dt_i - medianDt| / medianDt > DefaultTsRelativeEpsilon`, distinct from the absolute Ts check); `TsSeconds` mismatch fails (preview and step share the default epsilon); import JSON with `TsSeconds=0` / omitted Ts / empty num / empty den / extra property / `InitialConditions=zi` / `timeBase=index` / `timeBase=timestamp` / omitted `timeBase` / omitted `method` / omitted `schemaVersion` / `schemaVersion=2` / `a[0]=0` / `method=Filter` fails; `filter([1],[0],VDC)` Lower fails `TF_DEN_LEADING_ZERO`; `TfImport.OutputChannelKey` maps to MetricDraft; `filter([0.5 0.5],[1],VDC)` Lower → `TransferFunctionAlgorithm` and Save emits `ApplyTransferFunctionStep`; `AcquireVoltageStep` with no limits still publishes Sample `ElapsedMs`; `BitSweepAcquireStep` still publishes `ElapsedMs` (unchanged); `PublishTimedSampleStep` with unset `ElapsedMs` as a TF sibling fails `TF_MISSING_ELAPSED`; Save does not add `LimitLow`/`LimitHigh`; `filtfilt([0.5 0.5],[1],VDC)` Lower → `Method=filtfilt`; nested `mean(filter([1],[1],VDC))` and `filter([1],[1],VDC)+1` fail parse or Lower (never Expressions); decompile restores num/den/`TsSeconds`/`Method`/`InputChannelKey` via `OpenTapStepKinds`; Expressions is not a dependency of a TF-only plan; Basic `package.xml` still has no Core.dll; `fft` still fails parse; `filtfilt` step is a sibling analyze not inside acquire; architecture: `TransferFunctionFilter` / `TransferFunctionGrid` have no Avalonia/Core/Ivi.Visa; `TransferFunctionTimeBase` is Authoring.Core-only (uses `StoredSample`) and is absent from Basic.
- Risks: MATLAB `filter` vs Direct Form II numerical differences — lock form and goldens, do not chase every toolbox default. Zero-state transients: document ignore-first-N or match MATLAB `zi=0`. `filtfilt` Gustafsson vs SciPy defaults — check in MATLAB R202x vectors, do not generate goldens from SciPy in CI unless labeled. Uniform `Ts` may not match acquire `IntervalMs` if the host used ingest time — Area 10 already prefers `ElapsedMs`; TF fails closed without it. Always-on `ElapsedMs` on `AcquireVoltageStep` is an **additive column**; update host tests that snapshot Sample column names **and** `OpenTapHostTests` cases that currently assert shape-only VDC `ElapsedMs is null` (e.g. `Three_column_sample_still_publishes_without_elapsed`) in this same PR.
...
Allowed: `+ - * / ^` and `.* ./ .^`, parentheses, comparisons `> >= < <= ==`, `&& ||`, indexing `x(1)`, `x(end)`, `x(a:b)`, and numeric row vectors `[0.5 0.5]` / `[0.5, 0.5]` / `[1]` (Area 11 coefficient literals; Area 3 may still reject them until `filter` is registered). Identifiers are `InputChannelKeys` (vectors if the bound series has more than one sample).
...
% Continuous design: sys = c2d(tf(numC, denC), Ts); do not paste continuous b,a into tf(..., Ts)
sys = tf(b, a, Ts);
...
- `filtfilt` inside `RepeatNode`: per-iteration buffer vs whole-series (v1: sibling analyze of the preceding acquire only). Preview `SeriesByMetric` must not concatenate Repeat iterations onto one ElapsedMs grid that restarts per iteration on the bench.
