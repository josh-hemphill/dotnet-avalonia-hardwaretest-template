...
- Fail closed when the series is shorter than MATLAB `filtfilt` allows (pad/trim policy). `RequireUniform` also fails when length < 2 (`median(diff)` is undefined).
