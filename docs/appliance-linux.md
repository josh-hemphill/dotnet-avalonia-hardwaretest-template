...
- **Identity:** Technician name is the PIV Authentication (9A), Digital Signature (9C), or Card Auth (9E) certificate subject. If the subject is missing or a generic slot label, an rfc822 SAN / email (or email-like UPN) is used. Printed Information name is last. UID/ATR remains the badge serial for same-card signing.
