...
Compile inputs: `run.json` (camelCase `TestRunRecord`), Typst inputs (`title`, `runId`, `planName`, `dutSerial`, `operatorName`, `attestationKind`, `attestationDetail`, `result`, …), and optional sample-driven charts via `sample-chart.typ`. `EmbedPlotsInReport` toggles chart notes. Certification export can require a badge attestation when `RequireAttestationBeforeExport` is on. PIV/card signing injects an ISO 32000 **PAdES-B-B** signature into `certification.pdf` (incremental `/Sig` dictionary, `Adobe.PPKLite` / `adbe.pkcs7.detached` CMS over the PDF `ByteRange`, private key never leaves the card). A `{kind}.attestation.json` sidecar stores the same CMS plus certificate metadata. Mock HMAC and presence-only remain detached sidecars (presence is a site-policy fallback, not a signature).
