...
4. Optionally contribute `HomeTiles` (same placement rules) and `WarmAsync` for deferred start work while the startup overlay is visible. A faulted `WarmAsync` does not skip other apps. `WarmAsync` is cooperative: the host cancels the token on shutdown. Do not swallow `OperationCanceledException`. Dispose any `DeviceTransfer` lease in `finally`.
