...
Chip/tap signing captures the badge, **recompiles** the certification PDF in memory with that party's name/serial/transport (so example reports show who certified), then either injects an ISO 32000 **PAdES-B-B** signature into `certification.pdf` (incremental `/Sig` dictionary, `Adobe.PPKLite` / `adbe.pkcs7.detached` CMS over the PDF `ByteRange`, private key never leaves the card) or falls back to a detached sidecar when site policy allows presence-only attestation. A `{kind}.attestation.json` sidecar stores the attestation metadata; for PAdES it also carries the same CMS plus certificate metadata. PIN prompts, failed signs, and Cancel leave the previous PDF and attestation in place. Regenerating without a badge overlay clears attestation so a prior party is not reprinted. Results detail shows the same Certification summary from `run.Attestations`.
