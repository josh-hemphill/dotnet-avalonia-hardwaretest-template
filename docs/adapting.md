...
Chip/tap signing captures the badge, **recompiles** the certification PDF in memory with that party's name/serial/transport (so example reports show who certified), then freezes that attested document as `runs/{runId}/issued/{kind}.pdf` (`RunReportArtifact.Role` = `issued`). The regenerable working copy at `runs/{runId}/{kind}.pdf` (`Role` = `working`) is left unchanged. Signing either injects an ISO 32000 **PAdES-B-B** signature into the issued PDF (incremental `/Sig` dictionary, `Adobe.PPKLite` / `adbe.pkcs7.detached` CMS over the PDF `ByteRange`, private key never leaves the card) or falls back to a detached sidecar when site policy allows presence-only attestation. A `{kind}.attestation.json` sidecar in the run folder stores the attestation metadata; for PAdES it also carries the same CMS plus certificate metadata. PIN prompts, failed signs, and Cancel leave both PDFs and the previous attestation in place. Results detail shows the same Certification summary from `run.Attestations`.
...
**Export / storage:** Results **Export to…** copies working and issued PDFs + `run.json` + optional `*.attestation.json` (+ optional CSV) to a chosen destination. When an issued copy exists it is exported as `{kind}.pdf` and the regenerable working PDF nests under `working/`. Print uses the issued PDF when present. Results lists both copies in-app (Working / Issued) so appliances do not need a file browser. The Export package dropdown is filled from (1) inserted removable media (`DriveType.Removable` on Windows; `/media/*/*` and `/run/media/*/*` on Linux), (2) Settings **Export directory** (`ExportDirectory` / `HARDWARETEST_EXPORT_DIRECTORY` / `--export-directory`), then (3) `{DataDirectory}/exports` labeled **Local exports** when nothing else is listed. Each row shows the kind and the path. Settings **Prefer removable media for export** (`PreferRemovableExport`) controls whether removable volumes are listed first. Home crash **Export support bundle** uses the same targets. Retention prunes completed `runs/` folders by age/count using the injected clock; free-space warn/critical gates Run and surfaces on the shell strip (Critical is non-dismissible).
