...
Chip/tap signing captures the badge, **recompiles** the certification PDF in memory with that party's name/serial/transport (so example reports show who certified), signs the hash of those bytes plus `run.json`, and **writes the PDF and sidecar only after** a successful presence or signature. PIN prompts, failed signs, and Cancel leave the previous PDF and attestation in place. Regenerating without a badge overlay clears attestation so a prior party is not reprinted. Results detail shows the same Certification summary from `run.Attestations`.
